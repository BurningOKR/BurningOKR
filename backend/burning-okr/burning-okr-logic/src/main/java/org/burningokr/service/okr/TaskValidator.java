...
public class TaskValidator {
  ...
  /**
   * Checks, whether a task is self-referencing or if its predecessor
   * is located in another collumn or has a different state
   */
  public Task validateTask(Task task, TaskRepository taskRepository) throws Exception {
    if (!task.hasPreviousTask()) { return null; }
    ...
    logger.info(
        "Validation - previous and current task have different ids");
