...
SELECT id, ('hibernate_sequence'), 0, title
FROM task_state;

-- Initial audited key results
INSERT INTO key_result_aud (id, rev, revtype, name)
SELECT id, (SELECT CAST(current_value AS int) FROM sys.sequences WHERE name = 'hibernate_sequence'), 0, name
FROM key_result;
...
SELECT id, (SELECT CAST(current_value AS int) FROM sys.sequences WHERE name = 'hibernate_sequence'), 0,       description, title, assigned_key_result_id, parent_task_board_id, task_state_id
FROM task;

-- Initial audited task users
INSERT INTO task_user_aud (task_id, rev, revtype, user_id)
SELECT task_id, (SELECT CAST(current_value AS int) FROM sys.sequences WHERE name = 'hibernate_sequence'), 0, user_id
FROM task_user;
