...
public class WebsocketTaskController {
  ...
  public void addTask(@DestinationVariable long unitId, @Valid TaskDto taskDto, Authentication authentication) {
    SecurityContextHolder.getContext().setAuthentication(authentication);
    log.debug(String.format("Websocket add Task dto: {id: %d, title: %s, description: %s, assignedUserIds: %s, assignedKeyResultId: %d, task board Id: %d, stateId: %d}", taskDto.getId(), taskDto.getTitle(), taskDto.getDescription(), taskDto.getAssignedUserIds(), taskDto.getAssignedKeyResultId(), taskDto.getParentTaskBoardId(), taskDto.getTaskStateId()));

    Task newTask = taskMapper.mapDtoToEntity(taskDto);
    try {
      Collection<TaskDto> createdAndUpdatedTasks = this.taskMapper.mapEntitiesToDtos(taskService.createTask(newTask, unitId));
  ...
  public void updateTask(@DestinationVariable long unitId, @Valid TaskDto taskDto, Authentication authentication) throws Exception {
  ...
  public void deleteTask(@DestinationVariable long unitId, TaskDto taskDto, Authentication authentication) {
    ...
    try {
      ...
      Collection<Task> updatedTasks = taskService.deleteTaskById(taskToDelete.getId(), unitId);
